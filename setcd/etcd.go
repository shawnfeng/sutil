...
func (m *EtcdInstance) Regist(ctx context.Context, path, val string, heatbeat time.Duration, ttl time.Duration) error {
	...
	go func() {
		for i := 0; ; i++ {
			var err error
			if isset {
				slog.Warnf("%s create idx:%d val:%s", fun, i, val)
				_, err = m.Api.Set(ctx, path, val, &client.SetOptions{
					TTL: ttl,
				})
				if err == nil {
				...
				slog.Infof("%s refresh ttl idx:%d val:%s", fun, i, val)
				_, err = m.Api.Set(ctx, path, "", &client.SetOptions{
					PrevExist: client.PrevExist,
					TTL:       ttl,
					Refresh:   true,
	...
	go func() {
		...
		for {
			...
			if !ok {
				...
				slog.Infof("%s update path:%s", fun, r.Node.Key)
				hander(r)
				backoff.Reset()
			}
		}
...
func (m *EtcdInstance) startWatch(ctx context.Context, chg chan *client.Response, path string) {
	...
	for i := 0; ; i++ {
		...
		if r != nil {
			...
			slog.Infof("%s init get action:%s nodes:%d index:%d path:%s resp:%s", fun, r.Action, len(r.Node.Nodes), r.Index, path, sresp)
		}
		...
		if err != nil {
			...
			slog.Infof("%s next get idx:%d action:%s nodes:%d index:%d after:%d path:%s", fun, i, resp.Action, len(resp.Node.Nodes), resp.Index, wop.AfterIndex, path)
