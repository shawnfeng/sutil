...
//err 是否处理有报错
//增加errCode: 自定义错误码
func CollectServV2(instance, servkey string, servid int, processor string, duration time.Duration, source int, funcName string, errCode int, err interface{}) {
	durlabels := buildSerLabelsV2(instance, servkey, servid, processor, source, funcName, errCode)
	DefaultMetrics.AddHistoramSampleCreateIfAbsent(metricDurationNameKeys, duration.Seconds(), durlabels, nil)
	var counterLabels []Label
	if err == nil {
		counterLabels = buildSerReqLabelsV2(instance, servkey, servid, processor, source, funcName, errCode, Status_succ)
	} else {
		counterLabels = buildSerReqLabelsV2(instance, servkey, servid, processor, source, funcName, errCode, Status_fail)
	}
	DefaultMetrics.IncrCounterCreateIfAbsent(metricReqNameKeys, 1.0, counterLabels)
}
func buildSerLabelsV2(instance, servkey string, servid int, processor string, source int, funcName string, errCode int) []Label {
	targetServerName := SafePromethuesValue(servkey)
	instance = SafePromethuesValue(instance)
	sid := strconv.Itoa(servid)
	return []Label{
		...
		{Name: Label_source, Value: strconv.Itoa(source)},
		{Name: Label_type, Value: processor},
		{Name: Label_errcode, Value: strconv.Itoa(errCode)},
	}
}
func buildSerReqLabelsV2(instance, servkey string, servid int, processor string, source int, funcName string, errCode int, status int) []Label {
	labels := buildSerLabelsV2(instance, servkey, servid, processor, source, funcName, errCode)
	labels = append(labels, Label{
		Name: Label_status, Value: strconv.Itoa(status),
	})
	return labels
