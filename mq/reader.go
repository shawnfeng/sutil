...
func NewGroupReader(ctx context.Context, topic, groupId string) (Reader, error) {
	...
	switch mqType {
	case MQTypeKafka:
		return NewKafkaReader(config.MQAddr, wrapperTopicFromContext(ctx, topic), groupId, 0, 1, 10e6, config.CommitInterval), nil
...
func NewPartitionReader(ctx context.Context, topic string, partition int) (Reader, error) {
	...
	switch mqType {
	case MQTypeKafka:
		reader := NewKafkaReader(config.MQAddr, wrapperTopicFromContext(ctx, topic), "", partition, 1, 10e6, 0)
		if len(offsetAt) == 0 {
